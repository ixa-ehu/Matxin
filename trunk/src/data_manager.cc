...
chunk text2chunk(wstring text) {
  ...
  unsigned int separator = text.find(L"/");
...
void init_chunkMovement(string fitxName) {
  ...
  //while (fitx.good()) {
    ...
    unsigned int sep1 = lerro.find(L'\t');
    unsigned int sep2 = lerro.find(L'\t', sep1+1);
    unsigned int sep3 = lerro.find(L'\t', sep2+1);
...
void init_verb_subcategoritation(string fitxName, config &cfg){
  ...
  //while (fitx.good()) {
    ...
    while (lerro.find(L"#") != wstring::npos) {
      ...
      unsigned int sep2 = subcat.find(L'/');
      unsigned int sep3 = subcat.find(L'/', sep2+1);
...
void init_verb_noum_subcategoritation(string fitxName, config &cfg) {
  ...
  while (getline(fitx, lerro)) {
    ...
    while (lerro.find(L"#") != string::npos) {
      ...
      unsigned int sep2 = current.find(L"/");
...
wstring get_node_order(wstring chunk_type, wstring nodes) {
  ...
  wstring current_pos;
  int current_order = 0;
  unsigned int delimiter = nodes.find(L')');
