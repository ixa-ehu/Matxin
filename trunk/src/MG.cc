...
wstring verb_generation(wstring lemma, wstring pos, wstring suf, wstring cas, wstring mi, wstring chunk_mi, wstring head_sem, bool is_last, bool &flexioned, config cfg) {
  wstring analisys, form, prefix, postposizio, pre_lemma, lemma_osoa;
  lemma_osoa = lemma;

  if (cfg.DoGenTrace) wcerr << lemma << L" " << pos << L" " << mi << L" " << cas << endl;

  for (size_t i=0; i<lemma.size(); i++) {
    if (lemma[i] == L'_') lemma[i] = L' ';
  }
  ...
  unsigned int position1 = lemma.rfind(L' ');
  unsigned int position2 = lemma.rfind(L'-');
  if (position1 != wstring::npos or position2 != wstring::npos) {
    if (position1 == wstring::npos or (position2 != wstring::npos and position2 > position1)) {
      position1 = position2;
    }
  ...
  if (mi.substr(0,10) == L"[ADI][SIN]") {pos = L"[ADI][SIN]";mi.erase(0,10);}
  if (mi.substr(0,10) == L"[ADI][ADP]") {pos = L"[ADI][ADP]";mi.erase(0,10);}
  if (mi.substr(0,5) == L"[ADL]") {pos = L"[ADL]";mi.erase(0,5);}
  if (mi.substr(0,5) == L"[ADT]") {pos = L"[ADT]";mi.erase(0,5);}

  wstring pre_gen = L"^" + lemma_osoa + pos + L"$";
  if (cfg.DoGenTrace) wcerr << pre_gen << endl;
  wstring lemmaMorf = fstp_pre_generation.biltrans(pre_gen);
  if (cfg.DoGenTrace) wcerr << lemmaMorf << endl;

  if (lemmaMorf[0] == L'^' and lemmaMorf[1] == L'@') lemmaMorf = lemma + pos + mi;
  else lemmaMorf = lemmaMorf.substr(1, lemmaMorf.size()-2) + mi;

  for (int i=0; i<int(lemmaMorf.size()); i++) {
    if (lemmaMorf[i] == L'\\') {
      lemmaMorf.erase(i,1);
      i--;
    }
  ...
  if (analisys.find(L"LemaMorf") != wstring::npos) analisys.replace(analisys.find(L"LemaMorf"), 8, lemmaMorf);
  if (analisys.find(L"Lema") != wstring::npos) analisys.replace(analisys.find(L"Lema"), 4, lemma);
  if (analisys.find(L"Num") != wstring::npos) analisys.replace(analisys.find(L"Num"), 3, chunk_mi);
  if (analisys.find(L"Kas") != wstring::npos) {analisys.replace(analisys.find(L"Kas"), 3, cas);flexioned=true;}

  for (int i=0; i<int(analisys.size()); i++) {
    if (analisys[i] == L'[') analisys[i]= L'<';
    if (analisys[i] == L']') analisys[i]= L'>';
    if (analisys[i] == L' ' ) {
      analisys.erase(i,1);
      i--;
  ...
  if (cfg.DoGenTrace) wcerr << form << endl << endl;
  return form;
}


wstring number_generation(wstring lemma, wstring pos, wstring suf, wstring cas, wstring mi, wstring head_sem, bool is_last, bool &flexioned, config cfg) {
  wstring analisys, form, prefix, postposizio, pre_lemma, lemma_osoa;
  ...
  if (is_last) {
    ...
    if (cfg.DoGenTrace) wcerr << prefix << L" " << lemmaMorf << L" " << mi << L" " << cas << head_sem << endl;
    ...
    if (analisys.find(L"LemaMorf") != wstring::npos) analisys.replace(analisys.find(L"LemaMorf"), 8, lemmaMorf);
    if (analisys.find(L"Lema") != wstring::npos) analisys.replace(analisys.find(L"Lema"), 4, lemma);
    if (analisys.find(L"Num") != wstring::npos) analisys.replace(analisys.find(L"Num"), 3, mi);
    if (analisys.find(L"Kas") != wstring::npos) analisys.replace(analisys.find(L"Kas"), 3, cas);
...
wstring date_generation(wstring lemma, wstring pos, wstring suf, wstring cas, wstring mi, wstring head_sem, bool is_last, bool &flexioned, config cfg) {
  wstring analisys, form, prefix, postposizio, pre_lemma, lemma_osoa;
  ...
  else {year_cas=L"[GEL]";year_last=is_last;}
  ...
  if (century != L"??")
    form = number_generation(century, L"[CN]", suf, century_cas, L"[NUMS]", head_sem, century_last, flexioned, cfg);

  if (year != L"??")
    form = number_generation(year, L"[Z]", suf, year_cas, L"[NUMS]", head_sem, year_last, flexioned, cfg);

  if (month != L"??"){
    if (form != L"") form += L" ";
    form += number_generation(month, L"[MM]", suf, month_cas, L"[NUMS]", head_sem, month_last, flexioned, cfg);
  }

  if (day != L"??") {
    if (form != L"") form += L" ";
    form += number_generation(day, L"[Z]", suf, day_cas, L"[NUMS]", head_sem, day_last, flexioned, cfg);
  }

  if (week_day != L"??") {
    if (form != L"") form += L", ";
    form += number_generation(week_day, L"[WD]", suf, week_day_cas, L"[NUMS]", head_sem, week_day_last, flexioned, cfg);
  }

  if (hour != L"??.??") {
    if (form != L"") form += L", ";
    form += number_generation(hour, L"[Z]", suf, hour_cas, L"[MG]", head_sem, hour_last, flexioned, cfg);
    if (meridiam != L"??") form += L" (" + meridiam + L")";
  }
...
wstring generation(wstring lemma, wstring pos, wstring suf, wstring cas, wstring mi, wstring head_sem, bool is_last, bool &flexioned, config cfg) {
  wstring analisys, form, prefix, postposizio, pre_lemma, lemma_osoa;
  lemma_osoa = lemma;

  if (cfg.DoGenTrace) wcerr << lemma << L" " << pos << L" " << suf << L" " << mi << L" " << cas << L" " << head_sem << endl;

  for (size_t i=0; i<lemma.size(); i++) {
    if (lemma[i] == L'_') lemma[i]= L' ';
  }

  if ((!is_last && suf == L"") || cas == L"") return lemma;
  ...
  if (cfg.DoGenTrace) wcerr << lemmaMorf << L" " << mi << L" " << cas << L" " << head_sem << endl;
  ...
  if (form.size() == 0 or form[0] == L'@' or
    ...
    form = fstp_nolex_generation.biltrans(analisys);
    form = form.substr(1, form.size()-2);
    if (cfg.DoGenTrace) wcerr << L"GEN nolex:" << form << endl;
    if (form.size() == 0 or form[0] == L'@' or
        form.find(L"<") != wstring::npos or form.find(L">") != wstring::npos) {
      form = lemma;
    }
...
wstring procNODE(xmlTextReaderPtr reader, wstring chunk_type, wstring cas, wstring cas_alloc, wstring cas_ref, wstring mi, wstring head_sem, int chunk_len, config cfg){
  wstring nodes;
  wstring tagName = getTagName(reader);
  int tagType=xmlTextReaderNodeType(reader);
  ...
  if (tagName == L"NODE" and tagType != XML_READER_TYPE_END_ELEMENT) {
    ...
    else if (attrib(reader, "pos") == L"[Z]" || attrib(reader, "pos") == L"[Zu]" || attrib(reader, "pos") == L"[Zm]" || attrib(reader, "pos") == L"[Zp]" || attrib(reader, "pos") == L"[Zd]") {
...
wstring procCHUNK(xmlTextReaderPtr reader, config cfg) {
  ...
  if (tagName == L"CHUNK" and tagType == XML_READER_TYPE_ELEMENT) {
    ...
    if (det_cas != L"" && det_cas.rfind(L"++") != wstring::npos) {
      ...
      if (cfg.DoGenTrace) wcerr << postposizio << L" " << cas << endl;
      postposizio = generation(postposizio, L"", cas, L"", mi, head_sem, true, flexioned, cfg);
      if (cfg.DoGenTrace) wcerr << postposizio << endl;
      cas = det_cas.substr(0, det_cas.rfind(L"++")+2) + postposizio;
      if (cfg.DoGenTrace) wcerr << cas << endl;
    }
